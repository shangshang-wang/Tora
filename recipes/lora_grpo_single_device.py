...
class LoRAGRPORecipeSingleDevice(FTRecipeInterface):
    """
    Single-device LoRA GRPO recipe for dense transformer-based LLMs such as Llama2 and Vision-Language models.
    This recipe is optimized for training on a single GPU.
    ...
    Features:
        - Activation Checkpointing. This can be controlled using the ``activation_checkpointing``
            ...
            come at the cost of training performance.
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        log.info("metric logger is initialized.")
        ...
        pass  # Single device - no barrier needed
        ...
        if "reward_functions" in cfg and isinstance(cfg.reward_functions, (ListConfig, list)):
            ...
            log.info(f"Found reward names for logging: {self.reward_names}")
    ...
    def _setup_model(
        ...
        # Single device - no FSDP sharding needed
    ...
    def _setup_data(
        ...
        """
        All data related setup happens here. Single device uses regular DataLoader.
        """
        ...
        log.info("Dataset and DataLoader are initialized.")
    ...
    def save_checkpoint(self, epoch: int) -> None:
        ...
        log.info("Saving checkpoint. This may take some time. Retrieving full model state dict...")
        ...
        log.info(f"Getting full model state dict took {time.perf_counter() - start:.2f} secs")
        ...
        if intermediate_checkpoint:
            ...
            log.info(f"Getting optimizer state dict took {time.perf_counter() - start:.2f} secs")
        else:
            opt_state_dict = None
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self._epochs_run, self.total_epochs):
            ...
            pass  # Single device - no sampler epoch setting
    ...
    def log_metrics(
        ...
        """
        # Single device - no need to reduce across GPUs
    ...
    def cleanup(self) -> None:
        ...
        # Single device - no process group to destroy
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    """
    # Single device training - utilize all available CPU cores
    training.set_torch_num_threads()

    config.log_config(recipe_name="LoRAGRPORecipeSingleDevice", cfg=cfg)

    recipe = LoRAGRPORecipeSingleDevice(cfg=cfg)
