...
POST_TRAIN_TYPE="lora"      # Options: full, lora, dora, dora_cache
...
export SHADOW_LORA_RANK=2
export SHADOW_LORA_ALPHA=4
export TARGET_LORA_RANK=1
export TARGET_LORA_ALPHA=2
export SHADOW_SYNC_EVERY_N_STEPS=150
export NUM_WARM_UP_STEPS=50
export CONFIG_NAME="${POST_TRAIN_TYPE}_lifted_grpo_qwen2_5_${MODEL_SIZE}_shadow_${SHADOW_LORA_RANK}_target_${TARGET_LORA_RANK}"
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "${POST_TRAIN_TYPE}" == "full" ]; then
        ...
        tune run --nproc_per_node "${GPU_COUNT}" full_lifted_grpo_distributed \
            --config qwen2_5/"${MODEL_SIZE}_full_lifted_grpo" \
            seed="${SEED}" \
            compile="${USE_COMPILE}"
    else
        # Select config file based on QLoRA usage
        CONFIG_SUFFIX=$( [ "${USE_QLORA}" == "true" ] && echo "qlora_lifted_grpo" || echo "lora_lifted_grpo" )
        ...
        tune run --nproc_per_node "${GPU_COUNT}" lora_lifted_grpo_distributed \
            --config "${CONFIG_FILE}" \
