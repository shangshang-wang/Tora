...
# Configuration
MODEL_SIZE="11B"             # Options: 11B
POST_TRAIN_TYPE="lora"      # Options: full, lora, dora, dora_cache
USE_QLORA="true"            # Options: true, false
USE_COMPILE="false"         # Options: true, false
SEED=42                     # Set a seed for reproducibility

export BASE_MODEL_PATH="${CKPT_DIR}/llama3_2_vision/Llama-3.2-${MODEL_SIZE}-Vision"
MODEL_PATH="meta-llama/Llama-3.2-${MODEL_SIZE}-Vision"

# Override config in recipe
export CONFIG_NAME="${POST_TRAIN_TYPE}_grpo_llama3_2_vision_${MODEL_SIZE}"
export BASE_MODEL_DIR="${CKPT_DIR}/llama3_2_vision"
export BASE_OUTPUT_DIR="${OUTPUT_DIR}/llama3_2_vision"
export MODEL_SIZE
...
if [ "${POST_TRAIN_TYPE}" == "full" ]; then
    ...
    tune run --nproc_per_node 2 full_grpo_distributed \
        --config llama3_2_vision/"${MODEL_SIZE}_full_grpo" \
        seed="${SEED}" \
        compile="${USE_COMPILE}"
else
    # Select config file based on QLoRA usage
    CONFIG_SUFFIX=$( [ "${USE_QLORA}" == "true" ] && echo "qlora_grpo" || echo "lora_grpo" )
    CONFIG_FILE="llama3_2_vision/${MODEL_SIZE}_${CONFIG_SUFFIX}"
